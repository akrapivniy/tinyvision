...
void vision_hide_search_dark (struct v_sframe *sframe, int low_y, int hi_y)
{
	...
	*y += (low_y * w);
	...
	printf ("vhide.dark_val = %d vhide.dark_dir = %d, y_avg = %d  \n", vhide.dark_val,vhide.dark_dir, y_avg); fflush(stdout);
}
	...
	*y += (low_y * w) / 4;
	for (cy = low_y; cy < hi_y; cy++)  
		for (cx = 0; cx < w; cx++)  {
			...
			c2 = (*y>>16) & 0xff;
			c3 = (*y>>24) & 0xff;
			if (c0 > y_max) {y_max = c0; cx_max=cx;}
			if (c1 > y_max) {y_max = c1; cx_max=cx;}
			if (c2 > y_max) {y_max = c2; cx_max=cx;}
			if (c3 > y_max) {y_max = c3; cx_max=cx;}
			y++;
	}
